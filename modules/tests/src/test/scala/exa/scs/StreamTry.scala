...
Noch Besser CatsEffectSuite als IOApp: nur ausgewählte "test" laufen lassen (auch generell ohne IO - auch ganz ohne IO möglich mit @org.junit.Test)
    - Achtung: als MUnit single test über gutter: notwendig   "class .... extends CatsEffectSuite" UND toplevel einziges Element im File!!
auch möglich SimpleIOSuite (z.B. PaymentClientSuite) - jedoch test nicht einzeln auswählbar über gutter
 */
...
class StreamTry extends CatsEffectSuite with ScalaCheckEffectSuite {
  ...
  test("first PropF test") {
    PropF.forAllF { (x: PaymentId) =>
      IO(x).start.flatMap(_.join).map(res => { println(s"Differenz: $x - $res"); () })
    }
  }
  ...
  def writeToSocketSyncEx[F[_]: Sync: MonadThrow](chunk: Chunk[String], handleEx: Boolean) = {
    val fa = Random.scalaUtilRandom(Sync[F]).flatMap { r =>
      r.nextInt.map { rn =>
        println(s"writeToSocketSyncEx :: Writing $chunk to socket")
        if (rn % 2 == 0) {
          throw new RuntimeException(s"writeToSocketSyncEx :: RuntimeException bei id $rn mit Chunk $chunk")
        }
        println(s"writeToSocketSyncEx :: Saved:  $chunk mit id $rn")
        rn
      }
    }
    if (handleEx) MonadThrow[F].handleErrorWith(fa)(e => Sync[F].delay(println(s"writeToSocketSyncEx :: Exception $e")) >> Sync[F].delay(43434343))
    else fa
  }
  ...
  val writeStreamAsyncEx = sourceStream
    ...
    .attempt
    .evalMap {
      case Left(error) => IO.println(s"writeStreamAsyncEx :: Nach attempt: Left Error: $error")
      case Right(id)   => IO.println(s"writeStreamAsyncEx :: Nach attempt: Right: $id")
    }
  ...
  val release = (conn: Int) => IO.println(s"writeResourceStreamAsyncEx :: release (Releasing connection to the database: $conn)")
  ...
  def slprint(i: Any, rand: Random[IO[*]]) =
    rand.betweenInt(100, 3000).flatMap(d => IO.sleep(d.milliseconds)) *> IO(println(i))

  val parEvalSt = Stream(1, 2, 3, 4, 6, 7, 8,
    9).covary[IO].parEvalMapUnordered(2)(i => Random.scalaUtilRandom[IO].flatMap(r => slprint(i, r)))
